...
var jsPsychOrthographicSeg = (function (jspsych) {
  'use strict';

  const info = {
      name: "orthographic-seg",
      parameters: {
        ...
        response_button_html: {
          ...
          letter_button_html: {
              ...
              default: '<button class="letter-btn">%choice%</button>',
              array: true,
          },
  ...
  class HtmlButtonResponsePlugin {
      ...
      trial(display_element, trial) {
        ...
        // Set this flag for whether participants should only be able to click in the prefix or suffix position
          var PFX_SFX_ONLY_ACTIVE = false;
          ...
          if(PFX_SFX_ONLY_ACTIVE === false){
            // If all between-letter buttons are clickable

            for (var i = 0; i < button_labels.length; i++){
                if (i % 2 == 0){
                    // For even indices (the letters), push the html for a disabled button.
                    buttons.push('<button class="letter-btn" disabled>%choice%</button>')               
                } else {
                    // For odd indices (the spaces), push the default button html.
                    buttons.push(trial.letter_button_html)
                }
            }

        }else{
          // If only the prefix and suffix slots are to be active
          // Disable all the buttons that aren't the prefix and the suffix buttons.

            // Identify which button indices correspond to the suffix and the prefix
            // (should always be the fourth item in the array and the fourth-last)
            var pfx_idx = 3
            var sfx_idx = button_labels.length - 4

            // Replace all other indices with the disabled button html.
            for (var i = 0; i < button_labels.length; i++){
                if (![pfx_idx, sfx_idx].includes(i)){
                    // For even indices (the letters), push the html for a disabled button.
                    buttons.push('<button class="letter-btn" disabled>%choice%</button>')               
                } else {
                    // For the prefix and suffix indices, push the default button html.
                    buttons.push(trial.letter_button_html)
                }
            }
          ...
          // This div displays the word.
          html += '<br><div id="jspsych-html-button-letter-btngroup">';
          for (var i = 0; i < button_labels.length; i++) {
